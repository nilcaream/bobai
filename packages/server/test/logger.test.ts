...
import { createLogger, runWithScope } from "../src/log/logger";
...
describe("logger", () => {
	...
	test("creates log directory and writes daily file", () => {
		...
		expect(content).toContain("INFO TEST global hello world");
	});
	...
	test("formats line as timestamp level system message", () => {
		...
		expect(content).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3} WARN AUTH global token expired\n$/);
	});
	...
	test("withScope adds scope to log line", () => {
		const logger = createLogger({ level: "debug", logDir: tmpDir });
		const scoped = logger.withScope("abcd1234");
	...
	test("default logger uses 'global' as scope", () => {
		const logger = createLogger({ level: "debug", logDir: tmpDir });
		logger.info("TEST", "startup");

		const files = fs.readdirSync(tmpDir).filter((f) => f.endsWith(".log"));
		const content = fs.readFileSync(path.join(tmpDir, files[0]), "utf8");
		expect(content).toContain("INFO TEST global startup");
	});

	test("withScope for subagent uses parent-child format", () => {
		const logger = createLogger({ level: "debug", logDir: tmpDir });
		const scoped = logger.withScope("aaa11111-bbb22222");
	...
	test("log line has fixed column count when split by space", () => {
		...
		logger.withScope("abcd1234").info("SYS", "with session");
		logger.withScope("abcd1234-efgh5678").info("SYS", "with subagent");
		...
		for (const line of lines) {
			...
			expect(["global", "abcd1234", "abcd1234-efgh5678"]).toContain(parts[4]);
		}
	});

	test("AsyncLocalStorage scope is used by logger when no explicit scope", () => {
		const logger = createLogger({ level: "debug", logDir: tmpDir });

		runWithScope("ctx12345", () => {
	...
	test("explicit withScope takes precedence over AsyncLocalStorage", () => {
		const logger = createLogger({ level: "debug", logDir: tmpDir });
		const scoped = logger.withScope("explicit1");

		runWithScope("fromctx01", () => {
