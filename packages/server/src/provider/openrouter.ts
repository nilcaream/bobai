...
export function createOpenRouterProvider(auth: OpenRouterAuth, _logger?: Logger): Provider {
	return {
		id: "openrouter",
		async *stream(options: ProviderOptions): AsyncGenerator<StreamEvent> {
			...
			for await (const event of parseSSE(response.body)) {
				...
				if (choice?.finish_reason) {
					...
					const tokenLimit = getProviderModelConfig("openrouter", options.model)?.contextWindow ?? 0;
					const display = formatProviderModelDisplay("openrouter", options.model, promptTokens);
