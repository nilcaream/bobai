...
export const powershellTool: Tool = {
	definition: {
		...
		function: {
			name: "powershell",
			description:
				"Execute a PowerShell script in the project directory. Returns stdout, stderr, and exit code. The command is executed via powershell.exe -Command. Use ; to chain commands or backticks (`) for line continuations. Prefer PowerShell over cmd for complex scripting or object manipulation.",
			parameters: {
				type: "object",
				properties: {
					command: {
						type: "string",
						description: "The PowerShell script to execute. Can be multiline.",
					},
					timeout: {
						type: "number",
	...
	compact(output: string, callArgs: Record<string, unknown>): string {
		...
		return `${COMPACTION_MARKER} ${removed} lines from powershell(${JSON.stringify({ command })}) omitted\n${tail}`;
	},
